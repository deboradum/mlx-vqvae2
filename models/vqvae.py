...
class VQVAE2(nn.Module):
    def __init__(self, encoder_h_dim, res_h_dim, num_res_layers, k, d, beta):
        ...
        self.encoder_top = Top_encoder(3, encoder_h_dim, res_h_dim, num_res_layers)
    ...
    def __call__(self, x):
        ...
        print("h_btm", h_btm.shape)
        print("h_top", h_top.shape)
        exit()

        # Top part of fig 2a.
        h_top = self.pre_quantization_conv_top(h_top)
        loss_term_1, loss_term_2, e_top, perplexity = self.quantizer(h_top)
        dec_top = self.decoder_top(h_top)

        # Middle part of fig 2a.
        h_btm = self.pre_quantization_conv_btm(mx.concatenate(h_btm, dec_top, axis=1))  # TODO: Axis correct?
        loss_term_1, loss_term_2, e_btm, perplexity = self.quantizer(h_btm)
        ...
        x_hat = self.decoder(mx.concatenate(e_btm, e_top, axis=1))  # TODO: axis correct?
